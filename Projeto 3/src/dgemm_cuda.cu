...
extern "C" double* dgemm_parallel_cuda(const double* A, const double* B, int M, int N, int K, int tile) {
    if (M <= 0 || N <= 0 || K <= 0 || !A || !B) {
        return NULL;
    }

    if (tile <= 0) tile = TILE_DIM;
    if (tile > 32) tile = 32; // limita a 32 por segurança
    ...
    dgemm_kernel<<<grid, block, shmemBytes>>>(dA, dB, dC, M, N, K);
