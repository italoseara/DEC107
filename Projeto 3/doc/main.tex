...
\begin{document}
	...
	Não há sobreposição de computação e comunicação na implementação atual, pois as chamadas \texttt{cudaMemcpy} são bloqueantes por padrão. Isso significa que a CPU aguarda a conclusão da transferência de dados entre a memória do host e a memória da GPU antes de prosseguir para a próxima instrução. Como resultado, durante a transferência de dados via PCIe, a GPU permanece ociosa, e durante a execução do kernel na GPU, o barramento PCIe também fica ocioso.

	\subsection{Validação da Correção dos Resultados}
	
	Vamos validar a versão sequencial comparando os resultados obtidos com a função \texttt{\ldots}. A implementação sequencial (rodando na CPU) serve como a referência (ground truth). Ela é, por definição, a maneira mais simples e verificável de calcular o resultado (neste caso, a multiplicação de matrizes).
	...
	As Figuras~\ref{fig:eficiencia_cuda} e~\ref{fig:eficiencia_cuda_optimized} mostram a eficiência das implementações com CUDA (básica e otimizada com memória compartilhada). Onde $P = Tiles \times Tiles$ (número de threads por bloco). Observa-se que a eficiência diminui muito com o aumento do tamanho das matrizes, o que é esperado devido à sobrecarga de comunicação e sincronização entre threads. No entanto, a eficiência ainda é maior em comparação com as versões de CPU (OpenMP e MPI) para matrizes menores, destacando a vantagem do paralelismo massivo oferecido pelas GPUs.

	\subsection{Testando os limites da GPU}

	A versao basica era superior a versão shared na maioria dos testes, então decidimos testar a versao shared com tiles de 32x32 (1024 threads por bloco) e a versão básica para tentar maximizar o uso da memoria e ver se haveria uma melhora significativa.
	...
	\begin{thebibliography}{9}
		...
		\bibitem{knuthwebsite} NVIDIA CUDA C Programming Guide, Disponível em: \url{https://docs.nvidia.com/cuda/}. Acesso em: 24 de Novembro de 2025.
